...
/* types (now include mode & clientName on station) */
type Cell = { type:'MT'|'OP'; startAt:string; duration:number; clientName:string;
              left?:number; done?:boolean };
type Map = Record<string,Record<number,Cell|undefined>>;
type PlanStep = { modality:string; status:'PENDING'|'ACTIVE'|'DONE'; left?:number };
type ClientMode = 'MT'|'OP'|'UNSPEC';
type PlanClient={ id:string; name:string; mode:ClientMode; steps:PlanStep[] };
...
export default function Board(){
  ...
  const [first,setFirst]=useState(''); const [lastI,setLastI]=useState('');
  const [opts,setOpts]=useState<string[]>([]); const [mode,setMode]=useState<ClientMode>('MT');
  /* toast */
  ...
  useEffect(()=>{
    socket.on('station:update',({category,index,data}:any)=>
      setData(p=>({...p,[category]:{...(p[category]??{}),[index]:data}})));

    socket.on('plan:list', (all:PlanClient[])=>setClients(all));
    socket.on('station:batch', (map: Map) => setData(map));
    socket.on('plan:update',(one:PlanClient)=>setClients(p=>p.map(c=>c.id===one.id?one:c)));
...
const queueFor = (mod: string) =>
  clients.filter(c =>
    c.steps.some(s => s.status === 'PENDING' && s.modality === mod)
  );
...
const firstWaitingClientId = (mod: string, want: 'MT' | 'OP') => {
  const queued = queueFor(mod).find(q =>
    q.mode === 'UNSPEC' || q.mode === want
  );
  return queued?.id;                       // might be undefined
};

  /* ─── emits ─── */
  const start=(c:string,i:number,t:'MT'|'OP',clientId?:string)=>{
    socket.emit('optim:start',{category:c,index:i,type:t,clientId},(ack:any)=>{
  ...
  const cell=(c:string,i:number)=>{
    const st=data[c]?.[i]; const busy=!!st; const done=busy&&st!.done;
    let bg='bg-white hover:bg-sky-50';
    if(busy&&!done) bg=st!.type==='MT'?'bg-orange-500 text-white':'bg-indigo-600 text-white';
    if(done)        bg='bg-red-600 text-white';

    return(
      <div key={i} className="relative">
        <button disabled={busy&&!done} onClick={()=>!busy&&setPick({c,i})}
                className={`w-40 h-20 border rounded flex flex-col items-center justify-center transition-colors ${bg}`}>
          {busy
            ?(<>
        ...
        {busy && <button onClick={()=>stopAck(c,i)}
                         className="absolute -top-2 -right-2 bg-red-700 text-white w-6 h-6 rounded-full">×</button>}
      </div>);
  };
...
{/* ───────────────── side-panel ───────────────── */}
<aside
  className="shrink-0 h-[45vh] ipad:h-screen overflow-y-auto border-t ipad:border-t-0 ipad:border-l
  ...
  {queueFor(pick.c).map(q => (
    <li key={q.id}>
      <button
  disabled={clientBusy(q.id)}           // <-- NEW: prevent double-booking
  onClick={() => start(
    pick.c!,
    pick.i as number,
    q.mode === 'UNSPEC' ? 'MT' : q.mode, // UNSPEC → MT, else keep real mode
    q.id
  )}
  className={`w-full flex justify-between px-2 py-1 border rounded
              ${clientBusy(q.id)
      ...
      {showIntake && <Backdrop onClose={()=>setShowIntake(false)}>
        <div onClick={e=>e.stopPropagation()}
          ...
          <input value={first} onChange={e=>setFirst(e.target.value)}
                 className="w-full border p-2 rounded bg-white" placeholder="First name"/>
          ...
          <div className="flex flex-wrap gap-3 text-sm">
            {OPT_LIST.map(opt=>(
              <label key={opt} className="flex items-center gap-1">
                <input type="checkbox" checked={opts.includes(opt)}
                       onChange={e=>setOpts(p=>e.target.checked?[...p,opt]:p.filter(x=>x!==opt))}/>
                {opt}
              </label>
            ))}
          </div>

          <button disabled={disabled} onClick={submitPlan}
                  className={`w-full py-2 rounded text-white ${disabled?'bg-emerald-400 opacity-50':'bg-emerald-600 hover:bg-emerald-700'}`}>
            Create Plan
          </button>
        </div>
      </Backdrop>}
...
function Btn({ col, txt, on }:{
  col: 'orange' | 'indigo'; txt:string; on:()=>void
}){
  const clr = {
  ...
  return (
    <button onClick={on}
      className={`px-8 py-4 ${clr} text-white rounded font-semibold`}>
      {txt}
    </button>
  );
}
...
function Toast({msg}:{msg:string}){return(<div className="fixed bottom-4 left-4 bg-red-500 text-white p-3 rounded">{msg}</div>);}